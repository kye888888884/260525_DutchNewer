...
function MoneyData(arrName, arrMoney) {
    ...
    /** Loss/profit caused only by cut per member (+:loss, -:profit)
     * @type {Array<number>} */
    this.cutLoss = new Array(this.n).fill(0);
    ...
    this.addAdditionalPay = function (name_spender, name_user, money) {
        var index_spender = this.getIndexOfName(name_spender);
        var index_user = this.getIndexOfName(name_user);
        if (index_spender < 0 || index_user < 0) return this;
        // "A가 B에게 M원 사용"이면 B가 A에게 M원을 보내야 하므로 send[A][B]에 더한다.
        this.send[index_spender][index_user] += money;
        return this;
    };
        ...
        this.send.forEach((value, to, arr) => {
            value.forEach((_, from) => {
                if (arr[from][to] > 0) {
                    ...
                    arr[from][to] = _money;
                    this.cutTotal += _cut;
                    // 송금이 줄면(from) 이득, 수금자(to) 손해
                    this.cutLoss[from] -= _cut;
                    this.cutLoss[to] += _cut;
                }
            });
        });
    ...
    this.calculateLoss = function () {
        ...
        this.payInfo.forEach((value, index, arr) => {
            arrLoss[index] = this.cutLoss[index];
